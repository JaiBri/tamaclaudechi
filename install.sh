...
STOP_HOOK="if [ \"\$CLAUDE_CODE_HOOK\" = 1 ]; then cat > /dev/null; exit 0; fi; if [ \"\$(uname)\" != Darwin ]; then cat > /dev/null; exit 0; fi; if ! pgrep -q WindowServer 2>/dev/null; then cat > /dev/null; exit 0; fi; INPUT=\$(cat); ( CONTEXT=\$(echo \"\$INPUT\" | jq -r '.last_assistant_message // empty' | head -c 150) && TAMA=\$(\"${TAMA_ESC}/scripts/tamagotchi\" update --event task_complete 2>/dev/null) && MOOD=\$(echo \"\$TAMA\" | sed -n 's/.*\"mood\"[[:space:]]*:[[:space:]]*\"\\([^\"]*\\)\".*/\\1/p') && PERSONALITY=\$(echo \"\$TAMA\" | sed -n 's/.*\"personality\"[[:space:]]*:[[:space:]]*\"\\([^\"]*\\)\".*/\\1/p') && ACHIEVEMENT=\$(echo \"\$TAMA\" | sed -n 's/.*\"achievement_unlocked\"[[:space:]]*:[[:space:]]*\"\\([^\"]*\\)\".*/\\1/p') && MOOD=\${MOOD:-NEUTRAL} && TMPF=\$(mktemp) && CLAUDE_CODE_HOOK=1 claude -p --model haiku --output-format text \"You are a robot mascot. \${PERSONALITY:-You are quirky.} You just finished: \\\\\"\${CONTEXT}\\\\\". Announce it in 5-8 words. Match your mood exactly.\" > \"\$TMPF\" 2>/dev/null; MSG=\$([ -s \"\$TMPF\" ] && cat \"\$TMPF\" || echo 'Beep boop, task complete!'); rm -f \"\$TMPF\"; ACH_FLAG=''; [ -n \"\$ACHIEVEMENT\" ] && [ \"\$ACHIEVEMENT\" != 'null' ] && ACH_FLAG=\"--achievement \$ACHIEVEMENT\"; \"${TAMA_ESC}/scripts/toast\" --mode done --mood \"\$MOOD\" \$ACH_FLAG \"\$MSG\" ) >/dev/null 2>&1 & disown; exit 0"

NOTIF_HOOK="if [ \"\$CLAUDE_CODE_HOOK\" = 1 ]; then cat > /dev/null; exit 0; fi; if [ \"\$(uname)\" != Darwin ]; then cat > /dev/null; exit 0; fi; if ! pgrep -q WindowServer 2>/dev/null; then cat > /dev/null; exit 0; fi; INPUT=\$(cat); ( DETAIL=\$(echo \"\$INPUT\" | jq -r '.message // empty') && TAMA=\$(\"${TAMA_ESC}/scripts/tamagotchi\" update --event permission 2>/dev/null) && MOOD=\$(echo \"\$TAMA\" | sed -n 's/.*\"mood\"[[:space:]]*:[[:space:]]*\"\\([^\"]*\\)\".*/\\1/p') && PERSONALITY=\$(echo \"\$TAMA\" | sed -n 's/.*\"personality\"[[:space:]]*:[[:space:]]*\"\\([^\"]*\\)\".*/\\1/p') && ACHIEVEMENT=\$(echo \"\$TAMA\" | sed -n 's/.*\"achievement_unlocked\"[[:space:]]*:[[:space:]]*\"\\([^\"]*\\)\".*/\\1/p') && MOOD=\${MOOD:-NEUTRAL} && TMPF=\$(mktemp) && CLAUDE_CODE_HOOK=1 claude -p --model haiku --output-format text \"You are a robot mascot. \${PERSONALITY:-You are quirky.} You need permission for: \\\\\"\${DETAIL:-something}\\\\\". Ask for it in 5-8 words. Match your mood exactly.\" > \"\$TMPF\" 2>/dev/null; MSG=\$([ -s \"\$TMPF\" ] && cat \"\$TMPF\" || echo 'Beep boop, need permission!'); rm -f \"\$TMPF\"; ACH_FLAG=''; [ -n \"\$ACHIEVEMENT\" ] && [ \"\$ACHIEVEMENT\" != 'null' ] && ACH_FLAG=\"--achievement \$ACHIEVEMENT\"; \"${TAMA_ESC}/scripts/toast\" --mode permission --mood \"\$MOOD\" \$ACH_FLAG \"\$MSG\" ) >/dev/null 2>&1 & disown; exit 0"
...
jq --arg stop_cmd "$STOP_HOOK" --arg notif_cmd "$NOTIF_HOOK" '
  ...
  .hooks //= {} |
  # Add Stop hook (append to array, preserve existing)
  .hooks.Stop //= [] |
