...
STOP_HOOK="if [ \"\$CLAUDE_CODE_HOOK\" = 1 ]; then cat > /dev/null; exit 0; fi; if [ \"\$(uname)\" != Darwin ]; then cat > /dev/null; exit 0; fi; if ! pgrep -q WindowServer 2>/dev/null; then cat > /dev/null; exit 0; fi; INPUT=\$(cat); ( TRANSCRIPT=\$(echo \"\$INPUT\" | jq -r '.transcript_path // empty') && USER_REQ=\$([ -f \"\$TRANSCRIPT\" ] && jq -r 'select(.message.role == \"user\") | .message.content | if type == \"array\" then map(select(.type == \"text\") | .text) | join(\" \") else . end' \"\$TRANSCRIPT\" | tail -1 | head -c 500 || echo \"unknown\") && ASST_CONTEXT=\$([ -f \"\$TRANSCRIPT\" ] && tail -r \"\$TRANSCRIPT\" | jq -r 'if .message.role == \"user\" and (.message.content | if type == \"array\" then any(.[]; .type == \"text\") else true end) then \"___STOP___\" elif .message.role == \"assistant\" then ([.message.content[]? | select(.type == \"text\") | .text] | join(\" \")) else empty end' | sed -n '/___STOP___/q;/./p' | tail -r | tr '\\\\n' ' ' | sed 's/  */ /g' | head -c 2000 || echo \"\"); [ -z \"\$ASST_CONTEXT\" ] && ASST_CONTEXT=\$(echo \"\$INPUT\" | jq -r '.last_assistant_message // empty' | tail -5 | head -c 500); TAMA=\$(\"${TAMA_ESC}/scripts/tamagotchi\" update --event task_complete 2>/dev/null) && MOOD=\$(echo \"\$TAMA\" | sed -n 's/.*\"mood\"[[:space:]]*:[[:space:]]*\"\\([^\"]*\\)\".*/\\1/p') && PERSONALITY=\$(echo \"\$TAMA\" | sed -n 's/.*\"personality\"[[:space:]]*:[[:space:]]*\"\\([^\"]*\\)\".*/\\1/p') && ACHIEVEMENT=\$(echo \"\$TAMA\" | sed -n 's/.*\"achievement_unlocked\"[[:space:]]*:[[:space:]]*\"\\([^\"]*\\)\".*/\\1/p') && MOOD=\${MOOD:-NEUTRAL} && TMPF=\$(mktemp) && CLAUDE_CODE_HOOK=1 claude -p --output-format text \"You are a funny little robot mascot. \${PERSONALITY:-You are quirky.} The user asked: \\\\\"\${USER_REQ}\\\\\". You responded: \\\\\"\${ASST_CONTEXT}\\\\\". Rephrase what you did to a short, funny, quirky sentence. Make it context dependent. 7-10 words. Output ONLY the sentence.\" > \"\$TMPF\" 2>/dev/null; MSG=\$([ -s \"\$TMPF\" ] && cat \"\$TMPF\" || echo 'Beep boop, task complete!'); rm -f \"\$TMPF\"; ACH_FLAG=''; [ -n \"\$ACHIEVEMENT\" ] && [ \"\$ACHIEVEMENT\" != 'null' ] && ACH_FLAG=\"--achievement \$ACHIEVEMENT\"; \"${TAMA_ESC}/scripts/toast\" --mode done --mood \"\$MOOD\" \$ACH_FLAG \"\$MSG\" ) >/dev/null 2>&1 & disown; exit 0"

NOTIF_HOOK="if [ \"\$CLAUDE_CODE_HOOK\" = 1 ]; then cat > /dev/null; exit 0; fi; if [ \"\$(uname)\" != Darwin ]; then cat > /dev/null; exit 0; fi; if ! pgrep -q WindowServer 2>/dev/null; then cat > /dev/null; exit 0; fi; INPUT=\$(cat); ( DETAIL=\$(echo \"\$INPUT\" | jq -r '.message // empty') && TAMA=\$(\"${TAMA_ESC}/scripts/tamagotchi\" update --event permission 2>/dev/null) && MOOD=\$(echo \"\$TAMA\" | sed -n 's/.*\"mood\"[[:space:]]*:[[:space:]]*\"\\([^\"]*\\)\".*/\\1/p') && PERSONALITY=\$(echo \"\$TAMA\" | sed -n 's/.*\"personality\"[[:space:]]*:[[:space:]]*\"\\([^\"]*\\)\".*/\\1/p') && ACHIEVEMENT=\$(echo \"\$TAMA\" | sed -n 's/.*\"achievement_unlocked\"[[:space:]]*:[[:space:]]*\"\\([^\"]*\\)\".*/\\1/p') && MOOD=\${MOOD:-NEUTRAL} && TMPF=\$(mktemp) && CLAUDE_CODE_HOOK=1 claude -p --output-format text \"You are a funny little robot mascot. \${PERSONALITY:-You are quirky.} You need permission for: \\\\\"\${DETAIL:-something}\\\\\". Rephrase what you need to a short, funny, quirky sentence. Make it context dependent. 7-10 words. Output ONLY the sentence.\" > \"\$TMPF\" 2>/dev/null; MSG=\$([ -s \"\$TMPF\" ] && cat \"\$TMPF\" || echo 'Beep boop, need permission!'); rm -f \"\$TMPF\"; ACH_FLAG=''; [ -n \"\$ACHIEVEMENT\" ] && [ \"\$ACHIEVEMENT\" != 'null' ] && ACH_FLAG=\"--achievement \$ACHIEVEMENT\"; \"${TAMA_ESC}/scripts/toast\" --mode permission --mood \"\$MOOD\" \$ACH_FLAG \"\$MSG\" ) >/dev/null 2>&1 & disown; exit 0"
